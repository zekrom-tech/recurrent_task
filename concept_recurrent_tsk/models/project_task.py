...
class ProjectTask(models.Model):
    ...
    recurrence_start_date = fields.Date(
        string="Start From",
        default=fields.Date.context_today,   # keeps today as default
        tracking=True
    )
    ...
    def _cron_create_recurring_tasks(self):
        ...
        for template in templates:
            try:
                ...
                # ---------- Begin robust locked/claimed flow ----------
                # We will lock the template row, read authoritative next_recurrence_date, check for existing occurrence,
                # compute next_date, then attempt an atomic UPDATE ... WHERE next_recurrence_date = occurrence_date RETURNING id.
                self.env.cr.execute("""
                    SELECT next_recurrence_date
                      FROM project_task
                     WHERE id = %s
                     FOR UPDATE
                """, (template.id,))
                ...
                if not row:
                    _logger.info("CRON: Template disappeared from DB id=%s", template.id)
                    continue

                occurrence_date_db = row[0]
                # convert to date object if DB adapter returns string
                if isinstance(occurrence_date_db, str):
                    try:
                        occurrence_date = fields.Date.from_string(occurrence_date_db)
                    except Exception:
                        occurrence_date = occurrence_date_db
                else:
                    occurrence_date = occurrence_date_db

                # If there's no occurrence_date (shouldn't normally happen) skip
                if not occurrence_date:
                    _logger.info("CRON: Template id=%s has no next_recurrence_date, skipping", template.id)
                    continue

                # Compute next_date from the authoritative occurrence_date
                ...
                if not next_date:
                    ...
                    template.message_post(body=_("This recurring task has been finished."))
                    _logger.info("CRON: No next date calculated (after locking), stopping template: %s", template.name)
                    continue

                # Prevent duplicates by checking if an occurrence for that date already exists
                existing_count = self.env['project.task'].sudo().search_count([
                    ('recurring_template_id', '=', template.id),
                    ('occurrence_date', '=', occurrence_date),
                ])
                if existing_count:
                    _logger.info("CRON: Occurrence already exists for template %s on %s — advancing next_recurrence_date", template.id, occurrence_date)
                    # advance the next_recurrence_date so it won't be re-processed
                    template.sudo().write({'next_recurrence_date': next_date})
                    # Handle 'after' stop logic: decrementing recurrence_left is handled after successful creation normally,
                    # but since occurrence already exists (someone else created it), we mimic advancement: reduce left if appropriate
                    if template.repeat_type == 'after':
                        new_left = (template.recurrence_left or template.repeat_number or 0) - 1
                        template.sudo().write({'recurrence_left': new_left})
                        _logger.info("CRON: Remaining repetitions for %s adjusted to: %s", template.name, new_left)
                        if new_left <= 0:
                            template.sudo().write({'recurring_task': False, 'next_recurrence_date': False})
                            template.message_post(body=_("This recurring task has been finished as the number of repetitions has been reached."))
                            _logger.info("CRON: Stopped template after repetitions: %s", template.name)
                    continue

                # Now attempt atomic compare-and-set to advance next_recurrence_date (claim)
                self.env.cr.execute("""
                    UPDATE project_task
                ...
                if not claimed:
                    _logger.info("CRON: Claim failed for template %s (likely processed by another worker)", template.id)
                    continue

                # Create the child now that we have claimed the occurrence
                name_date_str = occurrence_date_db.strftime("%Y-%m-%d") \
                    if hasattr(occurrence_date_db, "strftime") else str(occurrence_date_db)
                default_vals = {
                ...
                try:
                    ...
                    # Unique constraint violation => someone created it in the tiny window; advance and continue
                    if getattr(e, 'pgcode', None) == UNIQUE_VIOLATION:
                        ...
                        self.env.cr.rollback()
                        self.env.cr.execute("SAVEPOINT before_raising")
                ...
                if template.repeat_type == 'until' and next_date and template.repeat_until and next_date > template.repeat_until:
                    template.sudo().write({'recurring_task': False, 'next_recurrence_date': False})
                    template.message_post(body=_("This recurring task has been finished as the end date has been reached."))
                    _logger.info("CRON: Stopped template due to end date: %s", template.name)
                elif template.repeat_type == 'after':
                    # If new_task was created, decrement; if it wasn't (duplicate), we decremented above in the exists branch.
                    if new_task:
                        new_left = (template.recurrence_left or template.repeat_number or 0) - 1
                        template.sudo().write({'recurrence_left': new_left})
                ...
                # ---------- End robust locked/claimed flow ----------
